...
std::pair<bool, SolutionPtr> IntraReinsertion::GetBestNeighbor(
  ...
  for (auto& vehicle : solution->vehicles()) {
    ...
    for (int i{1}; i < route_size - 1; i++) {
      for (int j{1}; j < route_size - 1; j++) {
        ...
        // std::cout << "CANDIDATO: Vehiculo " << vehicle->id() << ", delete: "
  ...
  if (IsLess(best_neighbor_time, solution->total_time())) {
    ...
    // 1]->route()[movement_.delete_pos]->id() <<
    //  " a la pos. " << solution->vehicles()[movement_.vehicle_id -
    //  1]->route()[movement_.insert_pos]->id() << std::endl;
...
double IntraReinsertion::GetNewTime(SolutionPtr new_solution,
                                    IntraReinsertionMovement movement,
  ...
  if (movement.delete_pos > movement.insert_pos) {
    ...
    delete_3 =
        instance->CalculateTime(zone_insert->id(), child_zone_insert->id());
  }
  double deleted_time = delete_1 + delete_2 + delete_3;
  // std::cout << "Quitamos " << parent_zone_delete->id() << " " <<
  // zone_delete->id() << std::endl; std::cout << "Quitamos " <<
  // zone_delete->id() << " " << child_zone_delete->id() << std::endl; std::cout
  // << "Quitamos " << zone_insert->id() << " " << child_zone_insert->id() <<
  // std::endl; Added
  insert_1 = instance->CalculateTime(parent_zone_delete->id(),
  ...
  if (movement.delete_pos > movement.insert_pos) {
    ...
    insert_3 =
        instance->CalculateTime(zone_delete->id(), child_zone_insert->id());
  }
  double added_time = insert_1 + insert_2 + insert_3;
  // std::cout << "Agreg " << parent_zone_delete->id() << " " <<
  // child_zone_insert->id() << std::endl; std::cout << "Agreg " <<
  // zone_insert->id() << " " << zone_delete->id() << std::endl; std::cout <<
  // "Agreg " << zone_delete->id() << " " << child_zone_insert->id() <<
  // std::endl;
  return old_time - deleted_time + added_time;
}

bool IntraReinsertion::IsLegalMovement(SolutionPtr solution,
                                       IntraReinsertionMovement candidate,
                                       std::shared_ptr<VRPInstance> instance) {
