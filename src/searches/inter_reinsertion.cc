...
std::pair<bool, SolutionPtr> InterReinsertion::GetBestNeighbor(
    ...
    for (int k{1}; k < route_size - 1;
      ...
      for (int l{0}; l < vehicles_size; l++) {
        ...
        for (int m{1}; m < other_route_size - 1;
          ...
          InterReinsertionMovement candidate = {vehicle->id(),
                                                other_vehicle->id(), k, m};
          InterReinsertionTimes new_times =
              GetNewTime(solution, candidate, instance);
  ...
  if (IsLess(best_neighbor_time, solution->total_time())) {
    ...
    auto& second_route =
        best_neighbor->vehicles()[movement_.insert_vehicle_id - 1]->route();
...
InterReinsertionTimes InterReinsertion::GetNewTime(
    SolutionPtr solution, InterReinsertionMovement movement,
  ...
  std::vector<ZonePtr> first_route =
      solution->vehicles()[movement.delete_vehicle_id - 1]->route();
  std::vector<ZonePtr> second_route =
      solution->vehicles()[movement.insert_vehicle_id - 1]->route();
  times.delete_route_time =
      solution->vehicles()[movement.delete_vehicle_id - 1]->TimeUsed();
  times.insert_route_time =
      solution->vehicles()[movement.insert_vehicle_id - 1]->TimeUsed();
  ...
  // Substracted
  double delete_1 =
      instance->CalculateTime(parent_zone_delete->id(), zone_delete->id());
  double delete_2 =
      instance->CalculateTime(zone_delete->id(), child_zone_delete->id());
  times.delete_route_time -= (delete_1 + delete_2);
  times.delete_route_time -= zone_delete->process_time();
  double delete_3 =
      instance->CalculateTime(parent_zone_insert->id(), zone_insert->id());
  times.insert_route_time -= delete_3;
  double deleted_time = delete_1 + delete_2 + delete_3;
  // std::cout << "Quitamos " << parent_zone_delete->id() << " " <<
  // zone_delete->id() <<" Costo: "<< delete_1 <<  std::endl; std::cout <<
  // "Quitamos " << zone_delete->id() << " " << child_zone_delete->id() <<"
  // Costo: " << delete_2 << std::endl; std::cout << "Quitamos " <<
  // parent_zone_insert->id() << " " << zone_insert->id() <<" Costo: " <<
  // delete_3 << std::endl; Added
  double insert_1 = instance->CalculateTime(parent_zone_delete->id(),
                                            child_zone_delete->id());
  double insert_2 =
      instance->CalculateTime(zone_delete->id(), zone_insert->id());
  double insert_3 =
      instance->CalculateTime(parent_zone_insert->id(), zone_delete->id());
  ...
  // std::cout << "Agreg " << parent_zone_delete->id() << " " <<
  // child_zone_delete->id() << " Costo: " << insert_1 << std::endl; std::cout
  // << "Agreg " << zone_delete->id() << " " << zone_insert->id() << " Costo: "
  // << insert_2 << std::endl; std::cout << "Agreg " << parent_zone_insert->id()
  // << " " << zone_delete->id() << " Costo: " << insert_3 << std::endl;
...
bool InterReinsertion::CheckMovement(SolutionPtr solution,
  ...
  auto first_route =
      solution->vehicles()[candidate.delete_vehicle_id - 1]->route();
  auto second_route =
      solution->vehicles()[candidate.insert_vehicle_id - 1]->route();
